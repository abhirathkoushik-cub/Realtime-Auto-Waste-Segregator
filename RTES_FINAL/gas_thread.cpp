...
public:
    void hasADS1115Sample(float sample) override {
        ...
        if (sample > 1.8f && systemState != SystemState::EMERGENCY) {
            systemState = SystemState::EMERGENCY;
            std::cout << "ALERT: Gas sensor voltage high! Emergency stop activated.\n";
        } else if (sample < 1.6f && systemState == SystemState::EMERGENCY) {
            systemState = SystemState::RUNNING;
            resumeFlag = true;
            std::cout << "Gas level safe. Resuming normal operation.\n";
        }
        ...
        double avg = static_cast<double>(total_emergency_time) / emergency_count;
...
void control_service()
{
    ...
    if (systemState == SystemState::EMERGENCY) {
        gpioWrite(MOSFET_GPIO, 0);
        gpioServo(SERVO_GPIO, 0);
    } else {
        if (state != lastState || resumeFlag) {
            auto timestamp = std::chrono::high_resolution_clock::now();
            long long time_us = std::chrono::duration_cast<std::chrono::microseconds>(
                timestamp.time_since_epoch()).count();

            if (resumeFlag) {
                std::cout << "[DEBUG] Resuming with current state = " << state << std::endl;
                resumeFlag = false;
            } else {
            ...
            if (state == 1) {
                gpioServo(SERVO_GPIO, 1500);
                gpioWrite(MOSFET_GPIO, 1);
                std::cout << "[servo] -> 90° + motor ON at " << time_us << " us\n";
            } else {
                gpioServo(SERVO_GPIO, 500);
                gpioWrite(MOSFET_GPIO, 1);
                std::cout << "[servo] -> 0° + motor ON at " << time_us << " us\n";
            }
            lastState = state;
        }
    }
    ...
    double avg = static_cast<double>(total_exec_time) / exec_count;
    long long this_jitter = std::abs(exec_time - avg);
...
int main()
{
    ...
    gpioSetMode(MOSFET_GPIO, PI_OUTPUT);
    gpioWrite(MOSFET_GPIO, 1);

    Sequencer sequencer;
    sequencer.addService(control_service, 1, 99, 20, new Service_Statistic, 1);
